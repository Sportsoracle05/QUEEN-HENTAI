...
let handler = async function (m, { conn, text, usedPrefix, command }) {
  ...
  if (user.registered === true) throw `💝 You are already registered with 💝 Queen Oracle 💝\n\n🇱🇰 Do you want to re-register to 💝 Queen Oracle 💝 ?🇱🇰\n\n 🇱🇰 Use this command to remove your register 🇱🇰\n*.unhentai* (Serial number)`
  if (!Reg.test(text)) throw `🇱🇰 Format incorrect\n\n 🇱🇰 Use this command of 💝 Queen Oracle 💝: *.qhw name.age*\n💝Example : *.qhw* Oracle.16`
  ...
  age = parseInt(age)
  if (age > 100) throw '💝 හම්මට නාකියා wants to play with 💝 Queen Oracle 💝 bot'
